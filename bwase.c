...
bwtint_t bwa_sa2pos(const bntseq_t *bns, const bwt_t *bwt, bwtint_t sapos, int ref_len, int *strand) {
    ...
    *strand = !is_rev;
    if (is_rev) {
        pos_f = pos_f + 1 < ref_len ? 0 : pos_f - ref_len + 1;
...
bwa_cigar_t *bwa_refine_gapped_core(bwtint_t l_pac, const ubyte_t *pacseq, int len, ubyte_t *seq, int ref_shift, bwtint_t *_rb, int *n_cigar) {
    ...
    if ((cigar32[*n_cigar - 1] & 0xf) == 1) {
        cigar32[*n_cigar - 1] = (cigar32[*n_cigar - 1] >> 4 << 4) | 3;
    } // change endding ins to soft clipping
    if ((cigar32[0] & 0xf) == 1) {
        cigar32[0] = (cigar32[0] >> 4 << 4) | 3;
    } // change beginning ins to soft clipping
...
char *bwa_cal_md1(
    int n_cigar, bwa_cigar_t *cigar, int len, bwtint_t pos, ubyte_t *seq,
...
void bwa_refine_gapped(const bntseq_t *bns, int n_seqs, bwa_seq_t *seqs, ubyte_t *_pacseq) {
    ...
    for (i = 0; i != n_seqs; ++i) {
        ...
        s->cigar = bwa_refine_gapped_core(bns->l_pac, pacseq, s->len, s->strand ? s->rseq : s->seq, s->ref_shift, &s->pos,
            &s->n_cigar);
        if (s->cigar == 0) {
            s->type = BWA_TYPE_NO_MATCH;
        }
...
void bwa_print_sam1(const bntseq_t *bns, bwa_seq_t *p, const bwa_seq_t *mate, int mode, int max_top2) {
    ...
    if (p->type != BWA_TYPE_NO_MATCH || (mate && mate->type != BWA_TYPE_NO_MATCH)) {
        ...
        if (p->type == BWA_TYPE_NO_MATCH) {
            ...
            j = pos_end(p) - p->pos;
        } // j is the length of the reference in the alignment
